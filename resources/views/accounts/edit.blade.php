    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <form action="{{ route('accounts.update', $account->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="email" class="appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" id="email" name="email" value="{{ old('email', $account->email) }}" required>
            </div>

            <div class="mb-4">
                <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Name</label>
                <input type="text" class="appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" id="name" name="name" value="{{ old('name', $account->name) }}" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select class="appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" id="usertype_id" name="usertype_id" required>
                    <option value="1" {{ old('usertype_id', $account->usertype_id) == 1 ? 'selected' : '' }}>Admin</option>
                    <option value="2" {{ old('usertype_id', $account->usertype_id) == 2 ? 'selected' : '' }}>User</option>
                </select>
            </div>
